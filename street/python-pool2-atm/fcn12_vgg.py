...
class FCN:
    ...
    def __init__(self, vgg16_npy_path="/home/dengdan/models/vgg/vgg16.npy", weight_decay = None):
    ...
    def build(self, rgb, train=False, num_classes=2, random_init_fc8=True,
        ...
        for image_index in range(pool2_shape[0]):
            scores = []
            s = self.text_score[image_index, ...]
            for channel in range(pool2_shape[-1]):
                scores.append(s)
            scores = tf.stack(scores, 2)
            scores = tf.expand_dims(scores, 0)
            self.text_score_elw.append(scores)
        
        self.text_score_elw = tf.concat(self.text_score_elw, 0, name = 'text_score_elementwise')
        self.process_output = self.pool2_l2_norm * self.text_score_elw
        
        self.process_output.set_shape(pool2_shape)
